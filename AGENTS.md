...
- Treat `heavenbase==0.1.0.5` as a normal PyPI runtime dependency.
...
- **Versioning:** `MAJOR.MINOR.PATCH.N[devK]` (currently `0.1.0.5`), aligned with HeavenBase when version bumps are intentional. Optional `devK` marks in-development snapshots.

...
def _run(repository: Path, command: str, *arguments: str, environment: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
    # Fixture repositories must not inherit the invoking Git hook repository.
...
def test_skill_sync_isolates_fixture_from_hook_environment(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    caller = tmp_path / "caller"
    caller.mkdir()
    _git(caller, "init")
    config = (caller / ".git" / "config").read_bytes()
    ...
    assert result.returncode == 0
    assert (caller / ".git" / "config").read_bytes() == config
    assert not (caller / ".git" / "index").exists()
    assert list((caller / ".git" / "refs" / "heads").iterdir()) == []

...
if [[ ! "${PROJECT_NAME}" =~ ^[A-Za-z0-9][A-Za-z0-9._[:space:]-]*$ ]]; then
    echo "error: --project-name contains unsupported characters" >&2
    exit 1
fi
...
while IFS= read -r -d '' file; do
    case "${file}" in
        ./.git/*|./.venv/*|./dist/*|./build/*|./.agents/skills/heaven-style/*|./scripts/rename.bash|./tests/test_rename.py)
            continue
            ;;
    esac
    if grep -Iq . "${file}"; then
        ...
        replace_text 'ProjectIdentity(name="Blueprint")' "ProjectIdentity(name=\"${PROJECT_NAME}\")" "${file}"
        ...
        replace_text "from ${OLD_IMPORT}" "from ${IMPORT_NAME}" "${file}"
        replace_text "import ${OLD_IMPORT}" "import ${IMPORT_NAME}" "${file}"
...
while IFS= read -r -d '' path; do
    ...
    find . -depth -name "*${OLD_IMPORT}*" \
        -not -path "./.git/*" \
        -not -path "./.venv/*" \
        -not -path "./dist/*" \
        -not -path "./build/*" \
        -not -path "./.agents/skills/heaven-style/*" \
        -print0
)

echo "[rename] renamed project to ${PROJECT_NAME} (dist=${DIST_NAME}, import=${IMPORT_NAME}, cli=${CLI_NAME})"
echo "[rename] review AGENTS.md and BLUEPRINT.md, then run uv lock and the complete repository gate"

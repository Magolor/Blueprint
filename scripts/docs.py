...
OPEN_STATUSES = {"draft", "ready", "active", "blocked", "postponed"}
...
def _validate_tasks(root: Path, errors: list[str]) -> list[dict[str, object]]:
    ...
    for position, raw_task in enumerate(tasks):
        ...
        if status == "postponed" and not str(task.get("resume_when") or "").strip():
            errors.append(f"TASK_RESUME: postponed task {task_id} needs an observable resume_when condition")

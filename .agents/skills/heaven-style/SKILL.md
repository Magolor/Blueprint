...
- **Docstring & Doc:** Require Python Google-style short descriptions with full Args/Returns/Yields structure. Use short, structured TSDoc in TypeScript; comments explain non-obvious constraints. Present accepted behavior without narrating the correction conversation, and keep minor edits proportional to the artifact's purpose. Keep canonical docs, examples, and generated artifacts consistent. Preserve conditions and exceptions in controlled prose. Separate current behavior, target, gap, and non-goal.

...
order: 110
category: project
keywords: [pytest, unit test, runnable example, demo script, coverage, happy path, edge case, error path]
...
Behavior changes need focused verification through the repository entrypoint. For new features, cover the happy path, one edge case, and one error path unless explicitly waived.
...
Use repo wrappers for external-database, LLM, or slow-marker suites; read `AGENTS.md` for marker and model policy.

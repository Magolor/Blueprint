...
Documentation is part of the maintained system, but prose is not automatically runtime truth. Each repository declares one canonical surface for users, one engineering entry point, one active task queue, one development log, and one temporary-note policy. Every durable fact has one normative home; other artifacts link to it.
...
- Development log: one declared path, newest-first ordering, and the newest `Next` reference to the active queue or `none`; older entries preserve historical task IDs.

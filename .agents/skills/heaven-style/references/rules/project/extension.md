...
- If registrations are removable, return an ownership token or disposer. Ensure that stale cleanup cannot remove a newer owner.
...
- Keep registration explicit. Treat side-effect imports as exceptions. A package import should not start I/O or mutate unrelated global state.

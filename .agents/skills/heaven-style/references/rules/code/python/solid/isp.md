---
name: python-solid-isp
description: Read when applying interface segregation to Python object and dependency boundaries.
---
...
```python
class FileStore(Backend):
    def get(self, key): ...
    def nearest(self, vector):
        raise NotImplementedError("file storage has no vector search")
```
...
class VectorSearch(Protocol):
    def nearest(self, vector: list[float]) -> list[Match]: ...

# The resolved search object really implements this separate role.
matches = search.nearest(vector)
```

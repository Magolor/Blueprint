...
- Keep pure, stateless helpers as functions/modules. Add a service/class/plugin only when it owns mutable state, lifecycle, registration, or replaceable behavior.
...
Source-level tests and workspace aliases cannot prove the packed package is usable.
...
- If the family is internal-only, is any implementation list confined to one composition root rather than runtime dispatch? If the family promises independent extensions, do bundled and external items use the same catalog and loader instead?

## Related rules

Also apply [architecture.md](architecture.md) for dependency direction and composition roots, [types.md](types.md) for import/type contracts, [async.md](async.md) for lazy integration errors and lifecycle, [docs.md](docs.md) for exported surfaces, and [environment.md](environment.md) for module profiles, package manager, and publish gates.

...
Coordinate an accepted delivery through assignments, review, integration, and verified handoff. The [Manager role](../../roles/manager.md) owns continuing delivery responsibility; this manual supplies the bounded coordination procedure. Small or tightly coupled work can remain local.
...
the outcome, boundaries, acceptance, and links. Use the [planning manual](../design/plan.md)
...
[compact protocol](../design/module.md#component-protocol).
Check attributes, method obligations, and public class count before assignment.
The architect owns the proposed design. The manager owns readiness and resolves
delegated decisions or routes reserved decisions with a concrete recommendation.
Use [intent and judgment](../../rules/project/work.md#interpret-intent-and-exercise-judgment)
to preserve the difference; uncertainty does not itself reserve the choice.
...
agent. Use the [file handoff](handoff.md) for subagents and parallel
...
Use [Extensive Review](../../workflows/review.md) for requested repeated review or substantial delivery that needs it.
...
Check the original acceptance before closeout, not just the most recent user
correction. A worker result, passing tests, and an integrated user path are
separate evidence. Name the next incomplete condition when any remains.

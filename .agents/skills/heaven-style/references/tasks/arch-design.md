...
related_rules: [overview, docs, review, ts-util, ts-architecture, ts-api, ts-types, ts-config, ts-modules, ts-async, ts-sql, ts-docs, ts-compat, ts-environment, model, oop, solid, types, docstring, files, clean, compat, extension, interfaces, test, environment]
---
...
- Use [TypeScript architecture](../rules/code/typescript/architecture.md) or [Python SOLID](../rules/code/python/solid.md) for SRP, OCP, LSP, ISP, and DIP boundary checks according to the target language.
...
- An open family that promises independent extensions should give bundled and external implementations the same public registration/selection contract and contract suite. Add persisted catalogs, acquisition, trust, or managed lifecycle only when the product explicitly promises those capabilities.
...
- Current behavior, accepted target, remaining gap, and non-goal are visibly distinct.

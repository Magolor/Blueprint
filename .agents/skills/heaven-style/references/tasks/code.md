...
related_rules: [overview, util, config, types, docstring, oop, model, solid, name, files, py, clean, error, sql, compat, ts-architecture, ts-types, ts-modules, ts-async, ts-docs, ts-environment, environment, format, test, docs, extension, interfaces]
---
...
Select rules by language: Python IDs and `ts-*` IDs are alternatives unless the change crosses a real Python/TypeScript boundary. Project rules apply to both but defer mechanics to the matched language environment.

## Fast Path

1. Read the user request, repo `AGENTS.md`, the project `environment` rule, target language/environment rules, declared lint/test entrypoints, docs, and nearby source before editing.
...
11. Run the target repository's declared style, lint, typecheck, test, build/package, and aggregate gates through `rtk`. Python uses its `uv`-backed wrappers; TypeScript preserves its checked-in manager/scripts and uses `ts-environment`.
...
- Code quality: follow Heaven architecture, repo guidance, and the matched language rules. Python uses the repository's declared utility/config/database/error owners or direct Python APIs when none exists; TypeScript uses strict boundaries, repository-owned platform/config layers, and explicit package/runtime contracts.
...
- API documentation: Python public APIs follow `docstring`; TypeScript published exports and stable seams follow `ts-docs`. Do not impose one language's comment format on the other.
...
- Legacy migration: follow the repository's public compatibility policy. Apply [compat.md](../rules/code/python/compat.md) only to matching Python/predecessor migrations.
...
Do not mark done until the implementation, tests, review, repository static/format gates, docs/example sync, generated artifacts, issue updates, and final human report are either completed or explicitly waived with the reason.

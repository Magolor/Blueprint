...
1. Read repo `AGENTS.md`, [test](../rules/project/test.md), the declared test scripts/configuration, package/runtime metadata, and any named issue/plan/report before editing tests.
...
5. Keep test code modular and generic. Share focused fixtures/factories. Parametrize equivalent cases. Assert observable outcomes. Avoid test names or assertions that describe unspecified behavior.
...
Do not mark done until all of these conditions hold:

- The retained suite has clear tier/tag ownership.
- Deleted tests are justified as non-contractual or redundant.
- Targeted daily tests pass.
- Any skipped broad validation is explicitly waived with the reason.

For substantial suite compression, save the keep/drop rationale in a linked plan or review/refactor report. Summarize closure in the development log.

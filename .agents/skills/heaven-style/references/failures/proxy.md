...
1. Check process proxy state before retrying: `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, `NO_PROXY`, lowercase variants, and tool-specific proxy config.
2. Assume a local VPN may be on during development. Make proxy behavior explicit per command instead of relying on ambient shell state.

...
2. For TypeScript, use the checked-in manager and package scripts. Inspect the active manager/runtime. Run the smallest frozen-install or declared gate. Do not introduce another lockfile or global tool to bypass the failure.
...
7. If shell activation fails, call the repository helper path instead of changing global PATH. Bash wrappers may resolve Python through an active virtualenv, repository `.venv`, `uv run python`, then system Python. Use the exact order declared by the target repository.
...
For connectivity or proxy ambiguity, use [proxy recovery](proxy.md).

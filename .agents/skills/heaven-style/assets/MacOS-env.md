...
Shared macOS maintenance playbook for Heaven-style agents working with this setup pattern. These operations are OS-wide guidance, not host-specific facts. Keep host identity, hardware details, serial numbers, UUIDs, private paths, and live state in ignored `assets/instance/*.local.md` files only when they are safe to record at all.
...
4. Preserve repository metadata. For greenfield JS/TS, treat Bun and pnpm as co-preferred; use Bun for compact Bun-native work and pnpm for Node-first libraries, services, or workspaces.
